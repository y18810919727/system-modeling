...
class TimeAwareRNN(nn.Module):
    ...
    def __init__(self, k_in, k_out, k_state, dropout=0., interpol="constant", **kwargs):
        ...
        self.cell = HOGRUCell(k_in, k_out, k_state, scheme='RK4', dropout=dropout, **kwargs)
        self.state0 = nn.Parameter(torch.zeros(k_state,), requires_grad=True)  # trainable initial state

    def forward_ori(self, inputs, state0=None, dt=None, **kwargs):  # potential kwargs: dt
        """
    ...
    def forward_prediction(self, external_input_seq, n_traj=16, max_prob=False, memory_state=None):
        with torch.no_grad():
            outputs, memory_state = self.forward_posterior(external_input_seq, observations_seq=None, memory_state=memory_state)
            predicted_seq = outputs['predict_seq']
    ...
    def forward_posterior(self, external_input_seq, observations_seq, memory_state=None):
        ...
        outputs, states = self.forward_ori(
            inputs=rearrange(external_input_seq, 'l b c -> b l c'),
            state0=state,
            dt=rearrange(dt, 'l b 1 -> b l 1')
        )
        outputs = torch.cat(
            [
                self.cell.Ly(state).unsqueeze(dim=1),
                outputs[:,:-1,:]
            ],
            dim=1
        )
        ...
        predict_seq, states = [rearrange(x, 'b l c -> l b c') for x in [outputs, states]]

        outputs = {
            'state': states,
            'predict_seq': predict_seq,
        }
        return outputs, {'state': states[-1]}

    def call_loss(self, external_input_seq, observations_seq, memory_state=None):
        outputs, memory_state = self.forward_posterior(external_input_seq, observations_seq, memory_state=memory_state)
        predict_seq = outputs['predict_seq']
        return {
            'loss': F.mse_loss(predict_seq, observations_seq),
            'kl_loss': 0,
            'likelihood_loss': 0
        }
