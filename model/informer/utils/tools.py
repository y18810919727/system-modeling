...
class StandardScaler():
    ...
    def fit(self, data, inpt=None, outpt=None):
        self.mean = data.mean(0)
        self.std = data.std(0)
        if inpt is not None:
            self.input_mean = self.mean[[inpt]]
            self.input_std = self.std[[inpt]]
        if outpt is not None:
            self.output_mean = self.mean[[outpt]]
            self.output_std = self.std[[outpt]]
