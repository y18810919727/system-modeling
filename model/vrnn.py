...
class VRNN(nn.Module):
    ...
    def forward_posterior(self, external_input_seq, observations_seq, memory_state=None):
        ...
        h, rnn_hidden_state = (torch.zeros(
            (batch_size, self.k), device=self.external_input_seq.device, dtype=self.external_input_seq.dtype
        ), None) if memory_state is None else (memory_state['hn'], memory_state['rnn_hidden'])
