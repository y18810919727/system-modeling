...
def create_narendra_li_datasets(**kwargs):
    # define output noise
    ...
    # test set input
    file_path = '/data/SE-VAE/data/nl/narendra_li_testdata.npz'
    ...
    dataset_train = pd.DataFrame(np.hstack((u_train, y_train)), columns=["u", "y"])
    dataset_val = pd.DataFrame(np.hstack((u_val, y_val)), columns=["u", "y"])
    dataset_test = pd.DataFrame(np.hstack((u_test, y_test)), columns=["u", "y"])
